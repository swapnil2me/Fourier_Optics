...
class MonochromaticField:
    ...
    def propagate_to_lens_focal_plane(self, focal_length, x_interval, y_interval):
        ...
        C = bluestein_fft2(self.E, x_interval[0] / (focal_length * self.λ), x_interval[1] / (focal_length * self.λ),
                           1 / self.dx,
                           y_interval[0] / (focal_length * self.λ), y_interval[1] / (focal_length * self.λ),
                           1 / self.dy)
        dfx = 1 / (self.Nx * self.dx)
        dfy = 1 / (self.Ny * self.dy)

        fx_zfft = bluestein_fftfreq(x_interval[0] / (focal_length * self.λ), x_interval[1] / (focal_length * self.λ),
                                    self.Nx)
        fy_zfft = bluestein_fftfreq(y_interval[0] / (focal_length * self.λ), y_interval[1] / (focal_length * self.λ),
                                    self.Ny)
        dfx_zfft = fx_zfft[1] - fx_zfft[0]
        dfy_zfft = fy_zfft[1] - fy_zfft[0]
        nn, mm = bd.meshgrid((bd.linspace(0, (self.Nx - 1), self.Nx) * dfx_zfft / dfx),
                             (bd.linspace(0, (self.Ny - 1), self.Ny) * dfy_zfft / dfy))
        ...
        self.E = C * ft_factor * bd.exp(1j * bd.pi / (self.λ * focal_length) * (
                    self.xx ** 2 + self.yy ** 2) + 1j * 2 * bd.pi / self.λ * focal_length) / (
                             1j * focal_length * self.λ)
        self.z += focal_length
    ...
    def interpolate(self, Nx, Ny):
        ...
        self.E = bd.array(fun_real(self.dx * (np.arange(Nx) - Nx // 2), self.dy * (np.arange(Ny) - Ny // 2)) + fun_imag(
            self.dx * (np.arange(Nx) - Nx // 2), self.dy * (np.arange(Ny) - Ny // 2)) * 1j)
    ...
    def __add__(self, Field):
        ...
        if ((self.extent_x == Field.extent_x) and (self.extent_y == Field.extent_y) and (self.Nx == Field.Nx) and (
                self.Ny == Field.Ny) and (self.λ == Field.λ)):
    ...
    from .visualization import plot_colors, plot_phase, plot_intensity, plot_longitudinal_profile_colors, \
        plot_longitudinal_profile_intensity
