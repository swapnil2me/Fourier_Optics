...
class ApertureFromLayout(DOE):
    def __init__(self, layout=None, phase_mask_path=None, image_size=None, phase_mask_format='hsv',
                 amplitude_mask_extent=[0, 1], simulation=None):
        ...
        if True:
            ...
            rescaled_img = rescale_img_to_custom_coordinates(img, self.image_size, simulation.extent_x,
                                                             simulation.extent_y, simulation.Nx, simulation.Ny)
            ...
            if amplitude_mask_extent != [0, 1]:
                t = bd.where((bd.abs(simulation.xx) < image_size[0] / 2) & (bd.abs(simulation.yy) < image_size[1] / 2),
                             t * (amplitude_mask_extent[1] - amplitude_mask_extent[0]) + amplitude_mask_extent[0],
                             bd.zeros(t.shape))
